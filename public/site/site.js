(() => {
  ...
  function home() {
    return `<main>${hero()}${communityHub()}
      ...
      <section class="section section-alt"><div class="container"><div class="section-head"><span class="eyebrow">Empieza en Valencia</span><h2>Todo preparado para tu primera sesión</h2><p class="section-lead">Desde el primer minuto tienes una ruta clara para conectar, informarte y entrar en rol.</p></div><div class="grid grid-3" data-reveal><article class="card feature-card feature-card-soft"><span class="feature-icon">01</span><h3>Lee la guía de inicio</h3><p>Aprende cómo crear tu personaje, entrar en emergencias o desarrollar tu historia como civil o maleante.</p><a class="text-link" href="/guia-inicio">Ver la guía →</a></article><article class="card feature-card feature-card-soft"><span class="feature-icon">02</span><h3>Comprueba el estado</h3><p>Antes de conectar, consulta el estado del servidor y cualquier mantenimiento anunciado por el equipo.</p><a class="text-link" href="/estado">Ver estado →</a></article><article class="card feature-card feature-card-soft"><span class="feature-icon">03</span><h3>Entra cuando estés listo</h3><p>Conecta FiveM directamente desde tu equipo y empieza tu primera historia en Valencia PD.</p><a class="text-link fivem-launch" href="${FIVEM_PROTOCOL}" data-fivem-launch>Jugar ahora →</a></article></div></div></section>
      <section class="section section-alt"><div class="container"><div class="section-head"><span class="eyebrow">Todo lo que necesitas</span><h2>Una comunidad hecha para quedarse</h2><p class="section-lead">Información clara, soporte rápido y un entorno preparado para disfrutar del rol.</p></div><div class="grid grid-3" data-reveal>
        ...
        <article class="card feature-card feature-card-soft"><span class="feature-icon">F</span><h3>Conexión directa</h3><p>Abre FiveM con un clic y entra directamente al servidor Valencia PD.</p><a class="text-link fivem-launch" href="${FIVEM_PROTOCOL}" data-fivem-launch>Jugar ahora →</a></article>
      </div></div></section>
  ...
  function galeria() { const captions=['Patrulla en servicio','Unidades de emergencia','Staff y comunidad','Detalle de patrulla','Vehículo de servicio','Trabajo en equipo','Operativo Valencia PD','Escena de intervención','Comunidad en acción']; return `<main>${pageHero('Galería','Vistazo en juego','Una selección de escenas y detalles visuales de Valencia PD.')}<section class="section"><div class="container"><div class="gallery-grid">${GALLERY.map((src,i)=>`<button type="button" class="gallery-item gallery-item-tall" data-gallery="${i}" aria-label="Ampliar ${captions[i]}">${assetImg(src,captions[i])}<span class="gallery-caption"><strong>${captions[i]}</strong><small>Valencia PD · ${String(i+1).padStart(2,'0')}</small></span></button>`).join('')}</div></div></section><div class="lightbox" id="lightbox"><img class="lightbox-img" id="lightbox-img" alt=""><button class="lightbox-close" id="lightbox-close" aria-label="Cerrar">✕</button><button class="lightbox-nav lightbox-prev" id="lightbox-prev" aria-label="Anterior">‹</button><button class="lightbox-nav lightbox-next" id="lightbox-next" aria-label="Siguiente">›</button></div></main>`; }

  function servicios() {
    const services = [
      ['Discord Valencia PD','Comunidad principal, tickets y anuncios.',DISCORD_CONNECT,'Conectar','discord'],
      ['Servidor FiveM',SERVER_IP,FIVEM_PROTOCOL,'Abrir FiveM','fivem'],
    ...
    return `<main>${pageHero('Comunidad','Nuestros servicios y redes','Todos los enlaces oficiales de Valencia PD reunidos aquí.')}
      <section class="section section-tight"><div class="container"><div class="grid grid-3">${services.map(s=>`<article class="card service-card"><div class="service-head"><span class="service-logo-file">${icon(s[4])}</span><h3 class="service-name">${s[0]}</h3></div><p class="service-desc">${s[1]}</p><a href="${s[2]}" ${s[2].startsWith('http') ? 'target="_blank" rel="noopener"' : ''} class="btn btn-discord btn-block service-join ${s[4] === 'fivem' ? 'fivem-launch' : ''}" ${s[4] === 'fivem' ? 'data-fivem-launch' : ''}>${s[3]}</a></article>`).join('')}</div></div></section>
    </main>`;
  }
